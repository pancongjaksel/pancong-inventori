...
CREATE TABLE item (
    ...
    reorder_point       NUMERIC(12,2),                     -- ambang stok menipis PER ITEM (bukan global) — NULL = belum di-set, gak dialert
...
CREATE TABLE users (
    ...
    password_hash   TEXT NOT NULL,                   -- bcrypt hash, JANGAN pernah simpan plaintext
    role            role_user NOT NULL,
    aktif           BOOLEAN NOT NULL DEFAULT true,
    token_versi     INTEGER NOT NULL DEFAULT 1,       -- naikkan angka ini buat paksa logout semua sesi (mis. abis ganti password)
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE device_gudang (
    ...
    gudang_id       INTEGER NOT NULL REFERENCES gudang(id),
    aktif           BOOLEAN NOT NULL DEFAULT true,
    token_versi     INTEGER NOT NULL DEFAULT 1,      -- naikkan buat cabut akses device ini (mis. HP ilang) tanpa hapus riwayat transaksinya
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- View: item yang stoknya di bawah reorder_point (alert stok menipis, Fase 1 scope)
-- Item dengan reorder_point NULL otomatis gak pernah muncul di sini (belum di-set admin).
