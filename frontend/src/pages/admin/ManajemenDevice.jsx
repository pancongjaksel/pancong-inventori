import { useEffect, useState } from 'react';
import { api, ApiError } from '../../api/client';

export default function ManajemenDevice() {
  const [devices, setDevices] = useState([]);
  const [kuota, setKuota] = useState([]);
  const [editKuota, setEditKuota] = useState({}); // { [gudangId]: value }
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [sukses, setSukses] = useState(null);
  const [prosesId, setProsesId] = useState(null);
  const [prosesKuotaId, setProsesKuotaId] = useState(null);

  function muatUlang() {
    setLoading(true);
    Promise.all([api.get('/device-gudang'), api.get('/device-gudang/kuota')])
      .then(([deviceData, kuotaData]) => {
        setDevices(deviceData);
        setKuota(kuotaData);
      })
      .catch((err) => setError(err instanceof ApiError ? err.message : 'Gagal memuat data.'))
      .finally(() => setLoading(false));
  }
  useEffect(muatUlang, []);

  async function toggle(device) {
    setProsesId(device.id);
    setError(null);
    try {
      await api.patch(`/device-gudang/${device.id}/${device.aktif ? 'cabut' : 'aktifkan'}`, {});
      muatUlang();
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Gagal ubah status.');
    } finally {
      setProsesId(null);
    }
  }

  async function simpanKuota(gudangId) {
    const nilai = editKuota[gudangId];
    if (nilai === undefined || nilai === '') return;
    setProsesKuotaId(gudangId);
    setError(null);
    try {
      await api.put(`/device-gudang/kuota/${gudangId}`, { maxDeviceQuota: Number(nilai) });
      setSukses('Kuota device tersimpan.');
      setEditKuota((prev) => {
        const next = { ...prev };
        delete next[gudangId];
        return next;
      });
      muatUlang();
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Gagal ubah kuota.');
    } finally {
      setProsesKuotaId(null);
    }
  }

  if (loading) return <p style={{ color: 'var(--warna-abu)' }}>Memuat...</p>;

  return (
    <div>
      {error && <div className="pesan-error">{error}</div>}
      {sukses && <div className="pesan-sukses">{sukses}</div>}

      <p className="label" style={{ marginBottom: 10 }}>Kuota device per gudang</p>
      {kuota.map((g) => (
        <div key={g.gudangId} className="kartu" style={{ marginBottom: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontWeight: 700 }}>{g.namaGudang}</div>
              <div style={{ fontSize: 13, color: 'var(--warna-abu)' }}>
                Terdaftar {g.currentCount} / {g.maxQuota} · Slot tersisa {g.availableSlots}
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <input
                type="number"
                className="input-teks"
                style={{ width: 70, height: 36 }}
                placeholder={String(g.maxQuota)}
                value={editKuota[g.gudangId] ?? ''}
                onChange={(e) => setEditKuota((prev) => ({ ...prev, [g.gudangId]: e.target.value }))}
              />
              <button
                className="tombol tombol--primer"
                style={{ width: 'auto', height: 36, padding: '0 12px', fontSize: 13 }}
                disabled={prosesKuotaId === g.gudangId || editKuota[g.gudangId] === undefined}
                onClick={() => simpanKuota(g.gudangId)}
              >
                Simpan
              </button>
            </div>
          </div>
        </div>
      ))}

      <p className="label" style={{ marginTop: 22, marginBottom: 10 }}>Semua device</p>
      <p style={{ fontSize: 13, color: 'var(--warna-abu)', marginTop: 0 }}>
        Device baru ditambahkan lewat proses scan QR (halaman Setup Device), bukan dari sini. Di sini cuma buat cabut akses (mis. HP hilang) atau aktifkan lagi.
      </p>

      {devices.length === 0 && (
        <p style={{ textAlign: 'center', color: 'var(--warna-abu)', marginTop: 40 }}>Belum ada device yang di-setup.</p>
      )}

      {devices.map((d) => (
        <div key={d.id} className="kartu" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, opacity: d.aktif ? 1 : 0.5 }}>
          <div>
            <div style={{ fontWeight: 700 }}>{d.nama_device}</div>
            <div style={{ fontSize: 13, color: 'var(--warna-abu)' }}>Gudang {d.nama_gudang}{!d.aktif && ' · Dicabut'}</div>
          </div>
          <button
            className={`tombol ${d.aktif ? 'tombol--bahaya' : 'tombol--sekunder'}`}
            style={{ width: 'auto', height: 36, padding: '0 14px', fontSize: 13 }}
            disabled={prosesId === d.id}
            onClick={() => toggle(d)}
          >
            {prosesId === d.id ? <span className="spinner" /> : d.aktif ? 'Cabut akses' : 'Aktifkan lagi'}
          </button>
        </div>
      ))}
    </div>
  );
}
