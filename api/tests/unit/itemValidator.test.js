...
describe('itemValidator.validasiUpdateItem', () => {
  ...
  test('lolos kalau minimal 1 field valid diisi', () => {
    assert.doesNotThrow(() => validasiUpdateItem({ reorderPoint: 10 }));
  });

  test('gagal kalau reorderPoint negatif', () => {
    assert.throws(() => validasiUpdateItem({ reorderPoint: -5 }));
  });

  test('reorderPoint null (buat hapus/reset) diizinkan', () => {
    assert.doesNotThrow(() => validasiUpdateItem({ reorderPoint: null }));
  });
