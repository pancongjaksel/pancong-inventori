...
async function buatItem(input) {
  const { kodeBarang, nama, kategori, satuan, gudangDefaultId, reorderPoint } = input;
  validasiBuatItem(input);

  try {
    const { rows } = await pool.query(
      `INSERT INTO item (kode_barang, nama, kategori, satuan, accounting_code, gudang_default_id, reorder_point)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, kode_barang, nama, kategori, satuan, status_aktif, reorder_point`,
      [kodeBarang.trim(), nama.trim(), kategori, satuan.trim(), ACCOUNTING_CODE[kategori], gudangDefaultId ?? null, reorderPoint ?? null]
    );
    return rows[0];
  } catch (err) {
    if (err.code === '23505') {
