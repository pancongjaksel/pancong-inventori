...
-- ----------------------------------------------------------------------------
-- AUTH — Magic Link (khusus Admin & Owner; Crew tidak login, lihat device_gudang)
-- ----------------------------------------------------------------------------

CREATE TABLE magic_link_token (
    id              BIGSERIAL PRIMARY KEY,
    user_id         INTEGER NOT NULL REFERENCES users(id),
    token_hash      VARCHAR(64) NOT NULL UNIQUE,   -- SHA-256 hex dari token asli (token asli TIDAK disimpan)
    expired_at      TIMESTAMPTZ NOT NULL,
    used_at         TIMESTAMPTZ,
    dibuat_dari_ip  VARCHAR(45),                   -- opsional, buat audit kalau ada yang mencurigakan
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX idx_magic_link_token_expired ON magic_link_token(expired_at) WHERE used_at IS NULL;
-- Token lama (expired atau sudah dipakai) sebaiknya dibersihkan berkala lewat
-- cron/scheduled job, bukan trigger DB (housekeeping, bukan aturan bisnis).
