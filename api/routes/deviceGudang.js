...
/** GET /api/device-gudang/kuota — status kuota device semua gudang (admin) */
router.get('/kuota', requireAdmin, async (req, res, next) => {
  try {
...
/** PUT /api/device-gudang/kuota/:gudangId — update kuota device 1 gudang (admin). Body: { maxDeviceQuota } */
router.put('/kuota/:gudangId', requireAdmin, async (req, res, next) => {
  try {
    const hasil = await updateKuotaGudang(Number(req.params.gudangId), Number(req.body.maxDeviceQuota));
    res.status(200).json({ sukses: true, data: hasil });
  } catch (err) {
    next(err);
  }
